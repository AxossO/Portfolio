...
const MobileMenu = ({ dark, setDark, turnOn }) => {
  ...
  return (
    <motion.div
      className={`${
        dark ? "yellow" : "black"
      } h-2/5  lg:hidden transition-all shadow-2xl rounded-b-3xl duration-200 ease-out mobile-menu  z-20 overflow-hidden pt-12 bg-black  w-full pl-0   top-0 fixed `}
      initial="hidden"
    ...
    >
      ...
      <div className=" py-3 px-0 text-xl z-40 absolute   w-full flex flex-col text-center items-center justify-cente">
        <ul
          className={`${
            turnOn ? "block" : "hidden"
          } overflow-hidden  w-full pt-6   tracking-widest`}
        >
          <Link
            activeClass="active"
            to="home"
